...
poetry run python "${CRON_SCRIPT_DIR}/cron__fetch_urls.py"
EXIT_CODE=$?
...
poetry run python "${CRON_SCRIPT_DIR}/cron__kaggle_job_yt_video_download.py"
EXIT_CODE=$?
if [ $EXIT_CODE -ne 0 ]; then
...
poetry run python "${CRON_SCRIPT_DIR}/cron__kaggle_job_yt_captions.py"
EXIT_CODE=$?
if [ $EXIT_CODE -ne 0 ]; then
...
poetry run python "${CRON_SCRIPT_DIR}/cron__kaggle_job_yt_info_collection.py"
EXIT_CODE=$?
if [ $EXIT_CODE -ne 0 ]; then
