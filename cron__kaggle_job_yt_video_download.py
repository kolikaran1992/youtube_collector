...
TEMPLATE_FILE = yt_config.KAGGLE_JOB_YT_VIDEO_DOWNLOAD_TEMPLATE_FILE
OUTPUT_SCRIPT_FILE = Path('yt_collector', 'yt_video_download_job_executable.py').as_posix()
JOB_NAME = 'yt-video-download-collector'
BASE_OUTPUT_DIR = yt_config.kaggle_output_dir_yt_video_download

# Initialize the Job Queues
try:
    # Source queue for this script: jobs that need video downloaded
    SOURCE_QUEUE = JobQueue(yt_config.yt_video_download_queue_dir)
    # Destination queue for this script: where jobs go after video download (user specified yt_caption_fetching_queue_dir)
    DESTINATION_QUEUE = JobQueue(yt_config.YT_VIDEO_DOWNLOAD_DESTINATION_QUEUE_DIR)
    MAX_URLS_TO_PROCESS = yt_config.KAGGLE_JOB_YT_VIDEO_DOWNLOAD_URLS_TO_PROCESS
...
def run_job():
    '''
    Pops jobs from the video download queue, generates the executable script,
    submits the job to Kaggle, and pushes the jobs to the destination queue.
    '''
    logger.info('🎬 Starting dynamic video download job generation and submission...')

    video_ids = []
    video_job_data = {} # To hold the original job data for pushing to the next queue

    # 1. Ingest jobs from SOURCE_QUEUE
    ...
    try:
        ...
        # 5. Push files to the next queue (Destination: YT_VIDEO_DOWNLOAD_DESTINATION_QUEUE_DIR)
        processed_count = 0
        ...
        for video_id, job_data in video_job_data.items():
            # Add the Kaggle job data under the specified key
            job_data['kaggle_job_yt_video_download'] = kaggle_job_data_to_add
            DESTINATION_QUEUE.push(video_id, job_data)
            processed_count += 1
        logger.info(f'✅ Successfully pushed {processed_count} jobs to the destination queue ({DESTINATION_QUEUE.queue_path.name}).')
        ...
        for video_id, job_data in video_job_data.items():
            ...
            if 'kaggle_job_yt_video_download' in job_data:
                del job_data['kaggle_job_yt_video_download']

            SOURCE_QUEUE.push(video_id, job_data)
            requeued_count += 1

        requeue_message = f"↩️ Successfully requeued {requeued_count} jobs to the SOURCE_QUEUE after submission failure."
