...
OUTPUT_SCRIPT_FILE = Path(
    "yt_collector", "yt_info_collection_job_executable.py"
).as_posix()
...
try:
    ...
    ARJAN_CODES_QUEUE = JobQueue(yt_config.arjan_codes_queue)
...
def send_slack_message_wrapper(message):
    send_slack_message_base(
        message=message,
        header=f"*YT-COLLECTOR-{uuid.uuid1().hex[:8]}: `{Path(__file__).name}`*",
    )
...
def run_job():
    ...
    logger.info(
        f"Scanning queue directory for up to {MAX_URLS_TO_PROCESS} video IDs..."
    )
    ...
    if videos_to_process_count == 0:
        abort_message = (
            "🛑 No new video IDs found in the queue. Job submission aborted."
        )
    ...
    logger.info(
        f"✅ Loaded job parameters: Processing {videos_to_process_count} videos."
    )
    ...
    final_script_content = substitute_placeholders(
        template_content, job_substitution_data
    )
    ...
    try:
        job_id = KernelLauncher().submit_job(
            ...
            slack_channel_id=(
                config.crongle.slack.channel_id
                if hasattr(config, "crongle") and hasattr(config.crongle, "slack")
                else None
            ),
            slack_bot_token=(
                config.slack.bot_token if hasattr(config, "slack") else None
            ),
        )
        ...
        for video_id, job_data in video_job_data.items():
            ...
            job_data["kaggle_job_yt_info_collection"] = (
                kaggle_job_data_to_add  # Updated key
            )
            if (
                job_data["channel_name"].lower()
                == yt_config.arjan_codes_channel_name.lower()
            ):
                logger.info(f"➡️ Pushing video {video_id} to Arjan Codes queue.")
                ARJAN_CODES_QUEUE.push(video_id, job_data)
            else:
                DESTINATION_QUEUE.push(video_id, job_data)
            processed_count += 1
        logger.info(
            f"✅ Successfully pushed {processed_count} jobs to the resting queue ({DESTINATION_QUEUE.queue_path.name})."
        )
