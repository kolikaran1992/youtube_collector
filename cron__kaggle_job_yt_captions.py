...
JOB_NAME = 'yt-caption-collector'
BASE_OUTPUT_DIR = yt_config.kaggle_output_dir_yt_captions

# Initialize the Job Queues
try:
    # Source queue for this script: jobs that need captions fetched
...
# Removed get_queue_files_and_video_ids and move_files_to_processing


# --- MAIN EXECUTION ---
def run_job():
    '''
    Pops jobs from the caption queue, generates the executable script, 
    submits the job to Kaggle, and pushes the jobs to the info fetching queue.
    '''
    logger.info('🎬 Starting dynamic caption job generation and submission...')
    
    video_ids = []
    video_job_data = {} # To hold the original job data for pushing to the next queue

    # 1. Ingest jobs from CAPTION_QUEUE (Source)
    ...
    kernel_type = 'cpu'
    polling_freq = yt_config.kaggle_job_poll_minutes_yt_caption
    ...
    try:
        ...
        # 5. Push files to the next queue (Destination: yt_info_fetching_queue_dir)
        processed_count = 0
        ...
        for video_id, job_data in video_job_data.items():
            ...
            job_data['kaggle_job_yt_captions'] = kaggle_job_data_to_add
            DESTINATION_QUEUE.push(video_id, job_data)
            processed_count += 1
        logger.info(f'✅ Successfully pushed {processed_count} jobs to the info fetching queue ({DESTINATION_QUEUE.queue_path.name}).')
        ...
        for video_id, job_data in video_job_data.items():
            ...
            if 'kaggle_job_yt_captions' in job_data:
                del job_data['kaggle_job_yt_captions']
                
            SOURCE_QUEUE.push(video_id, job_data)
            requeued_count += 1
        
        requeue_message = f"↩️ Successfully requeued {requeued_count} jobs to the CAPTION_QUEUE after submission failure."
